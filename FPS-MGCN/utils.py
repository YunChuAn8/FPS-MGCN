...
def clustering_acc_f1(y_true, y_pred, return_aligned=False):
    """
    用匈牙利算法做最佳标签匹配后，计算 acc 和 macro-F1
    y_true: 一维数组，可以是字符串或数字
    y_pred: 一维数组，聚类得到的标签（数字）
    """
    y_true = np.asarray(y_true)
    y_pred = np.asarray(y_pred)

    # 把真实标签和聚类标签都映射成从 0 开始的整数索引
    classes_true, y_true_idx = np.unique(y_true, return_inverse=True)
    classes_pred, y_pred_idx = np.unique(y_pred, return_inverse=True)
    ...
    for i in range(y_pred_idx.size):
        w[y_pred_idx[i], y_true_idx[i]] += 1
    ...
    # 比 np.vectorize(mapping.get) 更稳，避免未匹配时得到 None
    y_pred_aligned = np.array([mapping.get(x, x) for x in y_pred])

    # acc
    acc = (y_pred_aligned == y_true).mean()
    # F1（宏平均，按你需求也可以改成 weighted）
...
def fourier_transform(x, alpha=1.0):
    return torch.cat([torch.cos(alpha * math.pi * x), torch.sin(alpha * math.pi * x)], dim=1)
